...
const arrCountriesLanguage = countries_data.flatMap(oc => oc.languages)
const arr2 = arrCountriesLanguage.reduce((acc, e) =>{
	acc[e] = (acc[e] || 0) + 1
	return acc
}, {})
...
//3
function mostPopulatedCoutries(countries, n){
	const a = countries.flatMap(oc => [[oc.name, oc.population]])
	const b = a.map(([country, population]) => ({country, population}))
	b.sort((a, b) => b.population - a.population)
	return b.slice(0, n)
}
